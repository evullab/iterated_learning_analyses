function plot_compare_line_len(subj_data,mod_data,mod_params,ci)

cc=linspecer(1);

num_mod=length(mod_data);

subj_line_len=sum(sum(subj_data(:,:,1:2),3))./sum(sum(subj_data,3));
...
for im = 1:num_mod
    ...
    if ~exist('ci','var')
        plot(mod_disp,subj_line_len,'.','Color',cc,'MarkerEdgeColor',cc,...
                           'MarkerFaceColor',cc,...
                           'MarkerSize',40,'LineWidth',4)
    else
        errorbar(mod_disp,subj_line_len,abs(ci(1,:)-subj_line_len),abs(ci(2,:)-subj_line_len), ...
    ...
    axes;
    h = title('Proportion Similar Lengths','FontSize',40,'Color',[.3 .3 .3]);
    set(gca,'Visible','off');
    set(h,'Visible','on');
