function curr_llk=calc_llk_line(param_fname,data,mle_samps,exp_params,mod_params,task_params)
% get log-likelihood of each recalled display given groupings and original
% targets

fname=fullfile(strcat(param_fname),strcat('line_llk','A',num2str(mod_params(1).line_ang),'L',num2str(mod_params(1).line_len),'.mat'));
    ...
    for id=1:exp_params.numDisp
        for ic=1:exp_params.numChains
            for ii=1:(exp_params.numIts-1)
                ...
                ind_llk=distribution_response_line(targ_xy,curr_group,guess_xy,mod_params,task_params);
                if sum(isnan(ind_llk))~=length(ind_llk)
                    curr_llk(id,ic,ii)=sum(ind_llk(~isnan(ind_llk)));
                end
